...
export const HeaderSubGroupByCard: React.FC<IHeaderSubGroupByCard> = observer((props) => {
  ...
  return (
    <div
      className={`relative flex w-full flex-shrink-0 flex-row items-center gap-2 rounded p-1.5 cursor-pointer`}
      onClick={() => toggleExpanded()}
    >
      <div className="flex h-[20px] w-[20px] flex-shrink-0 items-center justify-center overflow-hidden rounded transition-all hover:bg-custom-background-80">
        {isExpanded ? <ChevronUpIcon width={14} strokeWidth={2} /> : <ChevronDownIcon width={14} strokeWidth={2} />}
      </div>

      <div className="flex h-[20px] w-[20px] flex-shrink-0 items-center justify-center overflow-hidden rounded">
