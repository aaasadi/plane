    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2">
        {VIEWS_LIST.map((chartView: any) => (
          <div
            key={chartView?.key}
            className={cn("cursor-pointer rounded-xs p-1 px-2 text-xs", {
      ...
      {showToday && (
        <button
          type="button"
          className="rounded-xs p-1 px-2 text-xs hover:bg-custom-background-80"
          onClick={handleToday}
        >
          {t("common.today")}
        </button>
      ...
      <button
        type="button"
        className="flex items-center justify-center rounded-xs border border-custom-border-200 p-1 transition-all hover:bg-custom-background-80"
