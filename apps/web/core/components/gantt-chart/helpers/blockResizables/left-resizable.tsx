  ...
  return (
    <>
      {(isHovering || isLeftResizing) && dateString && (
        <div className="absolute flex text-xs font-normal text-custom-text-300 h-full w-32 -start-36 justify-end items-center">
      ...
      <div
        ...
        className="absolute -start-1.5 top-1/2 -translate-y-1/2 z-[6] h-full w-3 cursor-col-resize rounded-md"
      />
      <div
        className={cn(
          "absolute start-1 top-1/2 -translate-y-1/2 h-7 w-1 z-[5] rounded-xs bg-custom-background-100 transition-all duration-300 opacity-0 group-hover:opacity-100",
          {
            "-start-1.5 opacity-100": isLeftResizing,
          }
        )}
      />
    </>
