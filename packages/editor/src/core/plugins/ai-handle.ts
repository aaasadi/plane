...
export const AIHandlePlugin = (options: SideMenuPluginProps): SideMenuHandleOptions => {
  ...
  const view = (view: EditorView, sideMenu: HTMLDivElement | null) => {
    ...
    const className =
      "grid place-items-center font-medium size-5 aspect-square text-xs text-custom-text-300 hover:bg-custom-background-80 rounded-xs opacity-100 !outline-none z-[5] transition-[background-color,_opacity] duration-200 ease-linear";
